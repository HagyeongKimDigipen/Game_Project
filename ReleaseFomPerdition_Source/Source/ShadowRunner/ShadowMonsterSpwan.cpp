	...
	if (isFinalRoom == false)
	{
		if (first == true)
		{
			...
			if ((abs(playerX - spawnerX) <= firstSpawnDistance) && (abs(playerY - spawnerY) <= firstSpawnDistance))
			{
				...
				GetWorld()->GetTimerManager().SetTimer(WaitHandle, FTimerDelegate::CreateLambda([&]() {  for (int i = 0; i < maxSpawnCount; i++) { particleComp[i]->DestroyInstance(); }}), spawnTime, false);
	...
	else
	{
		...
		if (first == true)
		{
			...
			if (playerX >= finalSpawnDistance && playerY >= 40.f)
			{
				...
				//GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &AShadowMonsterSpwan::particleMake, 3, true, 0);
				if (Particle_ability)
				{
					for (int i = 0; i < finalLocation.size(); i++)
					{
						particleComp[i] = UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), Particle_ability, finalLocation[i]);
					}
				}

				GetWorld()->GetTimerManager().SetTimer(WaitHandle, FTimerDelegate::CreateLambda([&]() {this->SpawnObject2(); for (int i = 0; i < finalLocation.size(); i++) { particleComp[i]->DestroyInstance(); } }), spawnTime, false);
