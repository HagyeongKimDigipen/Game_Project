	...
	if (isFinalRoom == false)
	{
		if (first == true)
		{
			...
			if ((abs(playerX - spawnerX) <= firstSpawnDistance) && (abs(playerY - spawnerY) <= firstSpawnDistance))
			{
				...
				//GetWorld()->GetTimerManager().SetTimer(WaitHandle, FTimerDelegate::CreateLambda([&]() { for (int i = 0; i < maxSpawnCount; i++) { particleComp[i]->DestroyInstance(); } this->SpawnObject(); }), spawnTime, false);
